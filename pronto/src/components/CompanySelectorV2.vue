...
export default {
  ...
  computed: {
    ...
    selectedCompany: {
      get() {
        const myStore = useMyStore();
        return myStore.selectedCompany;
      },
      set(value) {
        const myStore = useMyStore();
        myStore.selectCompany(value);
      }
    },
    selectedFrequency: {
      get() {
  ...
  setup() {
    ...
    const handleCompanyChange = () => {
      if (myStore.selectedCompany) {
        const selectedCompanyObj = myStore.Companies.find(company => company.id === myStore.selectedCompany);
    ...
    const fetchCompanies = async () => {
      if (authStore.isAuthenticated) {
        await myStore.fetchCompanies();
      }
    };

    // Fetch Companies when the component is first mounted
    onMounted(fetchCompanies);

    // Reactively fetch Companies when the authentication state changes
    watch(() => authStore.isAuthenticated, fetchCompanies);
    return {
      isAuthenticated,
      handleCompanyChange,
    };
  ...
  methods: {
    submit() {
      ...
      if (!this.isSubmitDisabled) {
        console.log('Selected Company ID:', myStore.selectedCompany);
